...
`r latexfig("arima-sim", output = is_latex())`  
```{r arima-sim, fig.cap="Simulación de un modelo autoregresivo."}
# Parametros
...
Ejemplo (`?arima.sim`):
`r latexfig("arima-sim2", output = is_latex())`  
```{r arima-sim2, fig.cap="Simulación de un modelo autoregresivo con errores con distribución *t* de Student."}
...
```{exercise, label="tsboot-lynx", name="Practical 8.1, Lynx data: Davison y Hinkley, 1997"}
...
`r latexfig("lynx-data", output = is_latex())`  

```{r lynx-data, fig.cap="Lynx data (logarithmic scale)."}
ts.plot(log(lynx))
lynx.ar <- ar(log(lynx))
lynx.ar$order
```
...
```{exercise, label="tsboot-beaver", name="Practical 8.2, Beaver data: Davison y Hinkley, 1997"}
...
`r latexfig("beaver-data", output = is_latex())`  

```{r beaver-data, fig.cap="Beaver data"}
# ?beaver
plot(beaver)
class(beaver)
str(beaver) 
...
```{exercise, label="tsboot-sunspot", name="Practical 8.3, Sunspot data: Davison y Hinkley, 1997"}
...
`r latexfig("sunspot", output = is_latex())`  

```{r sunspot, fig.cap="Sunspot data (yearly numbers)."}
...
del paquete `forecast`: `r latexfig("co2", output = is_latex())`

```{r co2, fig.cap="Datos de co2 (1990-1997) y simulación condicional (a partir de las observaciones desde 1990 hasta 1996)."}
library(forecast)
# ?co2
...
```{r co22, fig.cap="Predicción de los valores de co2 y simulación condicional (ambas a partir de las observaciones entre 1990 y 1996)."}
plot(forecast(fit, h=12*4), col="blue")
lines(ry, col="red")
```
`r latexfig("co22", output = is_latex())`
