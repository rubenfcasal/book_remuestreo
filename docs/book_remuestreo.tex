...
Carlo}\label{distribuciuxf3n-bootstrap-aproximada-por-monte-carlo}
...
Como se mostró en la Sección \ref{intro-implementacion}, el paso 1 se
...
Como ejemplo adicional podemos comparar la aproximación de la
distribución bootstrap del estadístico con la aproximación \(t_{n-1}\)
basada en normalidad.
...
\KeywordTok{abline}\NormalTok{(}\DataTypeTok{v =}\NormalTok{ pto_crit)}
\KeywordTok{curve}\NormalTok{(}\KeywordTok{dt}\NormalTok{(x, n}\OperatorTok{-}\DecValTok{1}\NormalTok{), }\DataTypeTok{add=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{)}
\NormalTok{pto_crit_t <-}\StringTok{ }\KeywordTok{qt}\NormalTok{(}\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{alfa}\OperatorTok{/}\DecValTok{2}\NormalTok{, n}\OperatorTok{-}\DecValTok{1}\NormalTok{)}
...
En este caso la distribución bootstrap del estadístico es más
asimétrica, lo que se traducirá en diferencias entre las estimaciones
por intervalos de confianza. Por ejemplo, podemos obtener la estimación
basada en normalidad mediante la función \texttt{t.test()}:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{t.test}\NormalTok{(muestra)}\OperatorTok{$}\NormalTok{conf.int}
\end{Highlighting}
\end{Shaded}
...
\(\frac{1}{2\sqrt{B}}\) (para más detalles sobre la convergencia de una
aproximación Monte Carlo ver p.e. el
\href{https://rubenfcasal.github.io/simbook/cap4.html}{Capítulo 4} de
Fernández-Casal y Cao, 2020).
...
\item
  ...
  Canty y posteriormente exportada a \texttt{R} (ver
  \href{http://cran.fhcrc.org/doc/Rnews/Rnews_2002-3.pdf}{Canty, 2002}).
  Este paquete es mucho más completo que el paquete \texttt{bootstrap},
  forma parte de la distribución estándar de \texttt{R} y es el que
  emplearemos como referencia en este libro (ver Sección
  \ref{intro-pkgboot}).
\end{itemize}
...
\KeywordTok{cor}\NormalTok{(Prestige}\OperatorTok{$}\NormalTok{income, Prestige}\OperatorTok{$}\NormalTok{prestige)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{statistic <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(data, i)\{}
\NormalTok{  remuestra <-}\StringTok{ }\NormalTok{data[i, ]}
  \KeywordTok{cor}\NormalTok{(remuestra}\OperatorTok{$}\NormalTok{income, remuestra}\OperatorTok{$}\NormalTok{prestige)}
\NormalTok{\}}

\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{B <-}\StringTok{ }\DecValTok{1000}
\NormalTok{res.boot <-}\StringTok{ }\KeywordTok{boot}\NormalTok{(Prestige, statistic, }\DataTypeTok{R =}\NormalTok{ B)}
\NormalTok{res.boot}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{estadistico_boot <-}\StringTok{ }\NormalTok{res.boot}\OperatorTok{$}\NormalTok{t }\OperatorTok{-}\StringTok{ }\NormalTok{res.boot}\OperatorTok{$}\NormalTok{t0 }
\KeywordTok{hist}\NormalTok{(estadistico_boot)}
\end{Highlighting}
\end{Shaded}
...
estimación}\label{relaciuxf3n-bootstrapjackknife-en-dicha-estimaciuxf3n}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ic_inf_boot_teor <-}\StringTok{ }\NormalTok{x_barra }\OperatorTok{-}\StringTok{ }\NormalTok{pto_crit_teor }\OperatorTok{*}\StringTok{ }\NormalTok{sigma}\OperatorTok{/}\KeywordTok{sqrt}\NormalTok{(n)}
\NormalTok{ic_sup_boot_teor <-}\StringTok{ }\NormalTok{x_barra }\OperatorTok{+}\StringTok{ }\NormalTok{pto_crit_teor }\OperatorTok{*}\StringTok{ }\NormalTok{sigma}\OperatorTok{/}\KeywordTok{sqrt}\NormalTok{(n)}
\NormalTok{IC_boot_teor <-}\StringTok{ }\KeywordTok{c}\NormalTok{(ic_inf_boot_teor, ic_sup_boot_teor)}
\KeywordTok{names}\NormalTok{(IC_boot_teor) <-}\StringTok{ }\KeywordTok{paste0}\NormalTok{(}\DecValTok{100}\OperatorTok{*}\KeywordTok{c}\NormalTok{(alfa}\OperatorTok{/}\DecValTok{2}\NormalTok{, }\DecValTok{1}\OperatorTok{-}\NormalTok{alfa}\OperatorTok{/}\DecValTok{2}\NormalTok{), }\StringTok{"%"}\NormalTok{)}
\NormalTok{IC_boot_teor}
\end{Highlighting}
\end{Shaded}
...
Una de las principales aplicaciones del bootstrap paramétrico es el
contraste de hipótesis que se tratará en la Sección
\ref{contrastes-parametricos}.
...
alrededor de la media muestral, \(\bar{X}\) (equivalentemente, la
distribución simetrizada es una mixtura de la distribución empírica
\(F_n\left( x \right)\) y de su versión ``reflejada''
\(1-F_n\left( 2\bar{X}-x \right)\) y se puede simular mediante el método
de composición; ver p.e. Fernández-Casal y Cao, 2020,
\href{https://rubenfcasal.github.io/simbook/m\%C3\%A9todo-de-composici\%C3\%B3n.html}{Sección
5.4}). Así el paso 1 resulta:
...
emplear implementaciones de otros paquetes (en la Sección \ref{npden-r}
se incluyen más detalles). Por ejemplo, considerando el conjunto de
datos \texttt{precip} (que contiene el promedio de precipitación, en
pulgadas de lluvia, de 70 ciudades de Estados Unidos), podríamos
utilizar el siguiente código {[}Figura \ref{fig:density}{]}:
...
\(\hat{f}_{h}\left( x \right)\) en dos pasos (empleando el denominado
método de composición; ver p.e. Fernández-Casal y Cao, 2020,
\href{https://rubenfcasal.github.io/simbook/m\%C3\%A9todo-de-composici\%C3\%B3n.html}{Sección
5.4}).
...
alternativo)}\label{ejemplo-muxe9todo-alternativo}
...
(e.g. \href{http://cran.fhcrc.org/doc/Rnews/Rnews_2002-3.pdf}{Canty,
2002}, y subsecciones siguientes). El primero consistiría en utilizar
directamente bootstrap uniforme, remuestreando las observaciones, y
sería adecuado para el caso de diseño aleatorio. La otra alternativa,
que podría ser más adecuada para el caso de diseño fijo, sería lo que se
conoce como \emph{remuestreo residual}, \emph{remuestreo basado en
modelos} o \emph{bootstrap semiparamétrico}. En esta aproximación se
mantienen fijos los valores de las variables explicativas y se
remuestrean los residuos. Una de las aplicaciones del bootstrap
semiparamétrico es el contraste de hipótesis en regresión, que se
tratará en la Sección \ref{contrastes-semiparametricos}.
...
\NormalTok{case.stat <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(data, i) \{}
\NormalTok{  fit <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(prestige }\OperatorTok{~}\StringTok{ }\NormalTok{income }\OperatorTok{+}\StringTok{ }\NormalTok{education, }\DataTypeTok{data =}\NormalTok{ data[i, ])}
  \KeywordTok{summary}\NormalTok{(fit)}\OperatorTok{$}\NormalTok{adj.r.squared}
\NormalTok{\}}

\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{boot.case <-}\StringTok{ }\KeywordTok{boot}\NormalTok{(Prestige, case.stat, }\DataTypeTok{R =} \DecValTok{1000}\NormalTok{)}
\NormalTok{boot.case}
\end{Highlighting}
\end{Shaded}
...
Como ya se comentó, en el caso de diseño fijo podemos realizar un
remuestreo de los residuos:
\[\mathbf{r} = \mathbf{Y} - X\hat{\mathbf{\beta}} = \mathbf{Y} - \hat{\mathbf{Y}}\]
obteniéndose las réplicas bootstrap:
\[\mathbf{Y}^{\ast} = \hat{\mathbf{Y}} + \mathbf{r}^{\ast}.\] Por
ejemplo, adaptando el código en Canty (2002) para este conjunto de
datos, podríamos emplear:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pres.dat <-}\StringTok{ }\NormalTok{Prestige}
\NormalTok{pres.dat}\OperatorTok{$}\NormalTok{fit <-}\StringTok{ }\KeywordTok{fitted}\NormalTok{(modelo)}
\NormalTok{pres.dat}\OperatorTok{$}\NormalTok{res <-}\StringTok{ }\KeywordTok{residuals}\NormalTok{(modelo)}

\NormalTok{mod.stat <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(data, i) \{}
\NormalTok{    data}\OperatorTok{$}\NormalTok{prestige <-}\StringTok{ }\NormalTok{data}\OperatorTok{$}\NormalTok{fit }\OperatorTok{+}\StringTok{ }\NormalTok{data}\OperatorTok{$}\NormalTok{res[i]}
\NormalTok{    fit <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(prestige }\OperatorTok{~}\StringTok{ }\NormalTok{income }\OperatorTok{+}\StringTok{ }\NormalTok{education, }\DataTypeTok{data =}\NormalTok{ data)}
    \KeywordTok{summary}\NormalTok{(fit)}\OperatorTok{$}\NormalTok{adj.r.squared}
\NormalTok{\}}

\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{boot.mod <-}\StringTok{ }\KeywordTok{boot}\NormalTok{(pres.dat, mod.stat, }\DataTypeTok{R =} \DecValTok{1000}\NormalTok{)}
\NormalTok{boot.mod}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pres.dat}\OperatorTok{$}\NormalTok{sres <-}\StringTok{ }\KeywordTok{residuals}\NormalTok{(modelo)}\OperatorTok{/}\KeywordTok{sqrt}\NormalTok{(}\DecValTok{1} \OperatorTok{-}\StringTok{ }\KeywordTok{hatvalues}\NormalTok{(modelo))}
\NormalTok{pres.dat}\OperatorTok{$}\NormalTok{sres <-}\StringTok{ }\NormalTok{pres.dat}\OperatorTok{$}\NormalTok{sres }\OperatorTok{-}\StringTok{ }\KeywordTok{mean}\NormalTok{(pres.dat}\OperatorTok{$}\NormalTok{sres)}
\end{Highlighting}
\end{Shaded}
...
simetrizado}{Método percentil-t simetrizado}}\label{icboot-perc-t-sim}
...
\KeywordTok{cor.test}\NormalTok{(Prestige}\OperatorTok{$}\NormalTok{income, Prestige}\OperatorTok{$}\NormalTok{prestige)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{res.boot <-}\StringTok{ }\KeywordTok{boot}\NormalTok{(Prestige, statistic, }\DataTypeTok{R =} \DecValTok{1000}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ht <-}\StringTok{ }\KeywordTok{h}\NormalTok{(res.boot}\OperatorTok{$}\NormalTok{t)}
\KeywordTok{hist}\NormalTok{(ht, }\DataTypeTok{freq =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{breaks =} \StringTok{"FD"}\NormalTok{,}
     \DataTypeTok{main =} \StringTok{"Distribución bootstrap en la escala transformada"}\NormalTok{)}
\KeywordTok{curve}\NormalTok{(}\KeywordTok{dnorm}\NormalTok{(x, }\DataTypeTok{mean=}\KeywordTok{mean}\NormalTok{(ht), }\DataTypeTok{sd=}\KeywordTok{sd}\NormalTok{(ht)), }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{, }\DataTypeTok{add =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{pvalor.ks <-}\StringTok{ }\KeywordTok{numeric}\NormalTok{(nsim)}
\NormalTok{pvalor.lil <-}\StringTok{ }\KeywordTok{numeric}\NormalTok{(nsim)}
\ControlFlowTok{for}\NormalTok{(isim }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{nsim) \{}
\NormalTok{  rx <-}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(nx, mx, sx)}
\NormalTok{  pvalor.ks[isim] <-}\StringTok{ }\KeywordTok{ks.test}\NormalTok{(rx, }\StringTok{"pnorm"}\NormalTok{, }\KeywordTok{mean}\NormalTok{(rx), }\KeywordTok{sd}\NormalTok{(rx))}\OperatorTok{$}\NormalTok{p.value}
\NormalTok{  pvalor.lil[isim] <-}\StringTok{ }\KeywordTok{lillie.test}\NormalTok{(rx)}\OperatorTok{$}\NormalTok{p.value}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
Como ejemplo estudiaremos el caso de contrastar una distribución
exponencial considerando 500 pruebas con muestras de tamaño 30 de una
\(Exp(1)\) y 200 réplicas bootstrap (para disminuir el tiempo de
computación).
...
\CommentTok{# Realizar contrastes}
\NormalTok{pvalor.ks <-}\StringTok{ }\KeywordTok{numeric}\NormalTok{(nsim)}
\NormalTok{pvalor.ks.boot <-}\StringTok{ }\KeywordTok{numeric}\NormalTok{(nsim)}
\ControlFlowTok{for}\NormalTok{(isim }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{nsim) \{}
\NormalTok{  rx <-}\StringTok{ }\KeywordTok{rexp}\NormalTok{(nx, ratex)}
\NormalTok{  pvalor.ks[isim] <-}\StringTok{ }\KeywordTok{ks.test}\NormalTok{(rx, }\StringTok{"pexp"}\NormalTok{, }\DecValTok{1}\OperatorTok{/}\KeywordTok{mean}\NormalTok{(rx))}\OperatorTok{$}\NormalTok{p.value}
\NormalTok{  pvalor.ks.boot[isim] <-}\StringTok{ }\KeywordTok{ks.exp.boot}\NormalTok{(rx, }\DataTypeTok{nboot =} \DecValTok{200}\NormalTok{)}\OperatorTok{$}\NormalTok{p.value}
\NormalTok{\}}
\CommentTok{# Generar gráficos}
\NormalTok{old.par <-}\StringTok{ }\KeywordTok{par}\NormalTok{(}\DataTypeTok{mfrow=}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{, }\DecValTok{2}\NormalTok{))}
\CommentTok{# Histograma}
\KeywordTok{hist}\NormalTok{(pvalor.ks, }\DataTypeTok{freq=}\OtherTok{FALSE}\NormalTok{, }\DataTypeTok{main =} \StringTok{"p-valor del test KS"}\NormalTok{)}
\KeywordTok{abline}\NormalTok{(}\DataTypeTok{h=}\DecValTok{1}\NormalTok{, }\DataTypeTok{lty=}\DecValTok{2}\NormalTok{)   }\CommentTok{# curve(dunif(x,0,1), add=TRUE)}
\CommentTok{# Distribución empírica}
\KeywordTok{curve}\NormalTok{(}\KeywordTok{ecdf}\NormalTok{(pvalor.ks)(x), }\DataTypeTok{type =} \StringTok{"s"}\NormalTok{, }\DataTypeTok{lwd =} \DecValTok{2}\NormalTok{, }
      \DataTypeTok{main =} \StringTok{'Tamaño del contraste KS'}\NormalTok{, }\DataTypeTok{ylab =} \StringTok{'Proporción de rechazos'}\NormalTok{, }
...
\StringTok{abline(a=0, b=1, lty=2)   # curve(punif(x, 0, 1), add = TRUE)}
\StringTok{# Histograma}
\StringTok{hist(pvalor.ks.boot, freq=FALSE, main = "p-valor del test KS-boot")}
\StringTok{abline(h=1, lty=2)   # curve(dunif(x,0,1), add=TRUE)}
\StringTok{# Distribución empírica}
\StringTok{curve(ecdf(pvalor.ks.boot)(x), type = "s", lwd = 2, }
\StringTok{      main = '}\NormalTok{Tamaño del contraste KS}\OperatorTok{-}\NormalTok{boot}\StringTok{', ylab = '}\NormalTok{Proporción de rechazos}\StringTok{', }
\StringTok{      xlab = '}\NormalTok{Nivel de significación')}
\KeywordTok{abline}\NormalTok{(}\DataTypeTok{a=}\DecValTok{0}\NormalTok{, }\DataTypeTok{b=}\DecValTok{1}\NormalTok{, }\DataTypeTok{lty=}\DecValTok{2}\NormalTok{)   }\CommentTok{# curve(punif(x, 0, 1), add = TRUE)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{cor}\NormalTok{(dogs}\OperatorTok{$}\NormalTok{mvo, dogs}\OperatorTok{$}\NormalTok{lvp)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{cor.test}\NormalTok{(dogs}\OperatorTok{$}\NormalTok{mvo, dogs}\OperatorTok{$}\NormalTok{lvp)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{cor.test}\NormalTok{(dogs}\OperatorTok{$}\NormalTok{mvo, dogs}\OperatorTok{$}\NormalTok{lvp, }\DataTypeTok{alternative =} \StringTok{"greater"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{statistic <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(data, i) }\KeywordTok{cor}\NormalTok{(data}\OperatorTok{$}\NormalTok{mvo, data}\OperatorTok{$}\NormalTok{lvp[i])}

\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{res.boot <-}\StringTok{ }\KeywordTok{boot}\NormalTok{(dogs, statistic, }\DataTypeTok{R =} \DecValTok{1000}\NormalTok{, }\DataTypeTok{sim =} \StringTok{"permutation"}\NormalTok{)}
\CommentTok{# res.boot}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{hist}\NormalTok{(res.boot}\OperatorTok{$}\NormalTok{t, }\DataTypeTok{freq =} \OtherTok{FALSE}\NormalTok{)}
\KeywordTok{abline}\NormalTok{(}\DataTypeTok{v =}\NormalTok{ res.boot}\OperatorTok{$}\NormalTok{t0, }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pval.greater <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(res.boot}\OperatorTok{$}\NormalTok{t }\OperatorTok{>=}\StringTok{ }\NormalTok{res.boot}\OperatorTok{$}\NormalTok{t0)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pval.less <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(res.boot}\OperatorTok{$}\NormalTok{t }\OperatorTok{<=}\StringTok{ }\NormalTok{res.boot}\OperatorTok{$}\NormalTok{t0)}
\NormalTok{pval <-}\StringTok{ }\DecValTok{2}\OperatorTok{*}\KeywordTok{min}\NormalTok{(pval.less, pval.greater)}
\NormalTok{pval}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{stat <-}\StringTok{ }\NormalTok{res}\OperatorTok{$}\NormalTok{fstatistic[}\DecValTok{1}\NormalTok{]}
\NormalTok{df <-}\StringTok{ }\NormalTok{res}\OperatorTok{$}\NormalTok{fstatistic[}\DecValTok{2}\NormalTok{]}
\NormalTok{dfr <-}\StringTok{ }\NormalTok{res}\OperatorTok{$}\NormalTok{fstatistic[}\DecValTok{3}\NormalTok{]}
\NormalTok{res}\OperatorTok{$}\NormalTok{fstatistic}
\end{Highlighting}
\end{Shaded}
...
Para aproximar la distribución de este estadístico bajo \(H_0\)
podríamos adaptar el bootstrap semiparamétrico\footnote{En este caso
  también podríamos emplear un contraste de permutaciones.} descrito en
la Sección \ref{boot-residual}:
...
\NormalTok{pres.dat <-}\StringTok{ }\NormalTok{Prestige}
\CommentTok{# pres.dat$fit0 <- mean(Prestige$prestige)}
\CommentTok{# pres.dat$fit0 <- predict(modelo0)}
\NormalTok{pres.dat}\OperatorTok{$}\NormalTok{res0 <-}\StringTok{ }\KeywordTok{with}\NormalTok{(Prestige, prestige }\OperatorTok{-}\StringTok{ }\KeywordTok{mean}\NormalTok{(prestige))}
\CommentTok{# pres.dat$res0 <- residuals(modelo0)}

\NormalTok{mod.stat <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(data, i) \{}
\NormalTok{    data}\OperatorTok{$}\NormalTok{prestige <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(data}\OperatorTok{$}\NormalTok{prestige) }\OperatorTok{+}\StringTok{ }\NormalTok{data}\OperatorTok{$}\NormalTok{res0[i]}
\NormalTok{    fit <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(prestige }\OperatorTok{~}\StringTok{ }\NormalTok{income }\OperatorTok{+}\StringTok{ }\NormalTok{education, }\DataTypeTok{data =}\NormalTok{ data)}
    \KeywordTok{summary}\NormalTok{(fit)}\OperatorTok{$}\NormalTok{fstatistic[}\DecValTok{1}\NormalTok{]}
\NormalTok{\}}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{hist}\NormalTok{(boot.mod}\OperatorTok{$}\NormalTok{t, }\DataTypeTok{breaks =} \StringTok{"FD"}\NormalTok{, }\DataTypeTok{freq =} \OtherTok{FALSE}\NormalTok{)}
\KeywordTok{curve}\NormalTok{(}\KeywordTok{pf}\NormalTok{(x, df, dfr, }\DataTypeTok{lower.tail =} \OtherTok{FALSE}\NormalTok{), }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{, }\DataTypeTok{add =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# pval <- mean(boot.mod$t >= boot.mod$t0)}
\NormalTok{pval <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(boot.mod}\OperatorTok{$}\NormalTok{t }\OperatorTok{>=}\StringTok{ }\NormalTok{stat)}
\NormalTok{pval}
\end{Highlighting}
\end{Shaded}
...
Adicionalmente, como se mostró en la Sección \ref{boot-residual}, se
puede emplear la modificación propuesta en Davison y Hinkley (1997, Alg.
6.3, p.~271) y remuestrear los residuos reescalados y centrados.
...
\CommentTok{# pres.dat$fit0 <- mean(Prestige$prestige)}
\CommentTok{# pres.dat$fit0 <- predict(modelo0)}
\CommentTok{# pres.dat$res <- residuals(modelo)}
\NormalTok{pres.dat}\OperatorTok{$}\NormalTok{sres <-}\StringTok{ }\KeywordTok{residuals}\NormalTok{(modelo)}\OperatorTok{/}\KeywordTok{sqrt}\NormalTok{(}\DecValTok{1} \OperatorTok{-}\StringTok{ }\KeywordTok{hatvalues}\NormalTok{(modelo))}
\NormalTok{pres.dat}\OperatorTok{$}\NormalTok{sres <-}\StringTok{ }\NormalTok{pres.dat}\OperatorTok{$}\NormalTok{sres }\OperatorTok{-}\StringTok{ }\KeywordTok{mean}\NormalTok{(pres.dat}\OperatorTok{$}\NormalTok{sres)}

\NormalTok{mod.stat <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(data, i) \{}
    \CommentTok{# data$prestige <- mean(data$prestige) + data$res[i]}
\NormalTok{    data}\OperatorTok{$}\NormalTok{prestige <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(data}\OperatorTok{$}\NormalTok{prestige) }\OperatorTok{+}\StringTok{ }\NormalTok{data}\OperatorTok{$}\NormalTok{sres[i]}
\NormalTok{    fit <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(prestige }\OperatorTok{~}\StringTok{ }\NormalTok{income }\OperatorTok{+}\StringTok{ }\NormalTok{education, }\DataTypeTok{data =}\NormalTok{ data)}
    \KeywordTok{summary}\NormalTok{(fit)}\OperatorTok{$}\NormalTok{fstatistic[}\DecValTok{1}\NormalTok{]}
\NormalTok{\}}
...
densidad}\label{estimaciuxf3n-no-paramuxe9trica-de-la-funciuxf3n-de-densidad}
...
medio}\label{sesgo-varianza-y-error-cuadruxe1tico-medio}
...
\subsection{Bootstrap y estimación del
sesgo}\label{bootstrap-y-estimaciuxf3n-del-sesgo}
...
Como se comentó en la Sección \ref{aproximacion-bootstrap} la ventana
\(g\) ha de ser asintóticamente mayor que \(h\) (de orden \(n^{-1/5}\))
...
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{n <-}\StringTok{ }\KeywordTok{length}\NormalTok{(x)}
\NormalTok{g <-}\StringTok{ }\NormalTok{h }\OperatorTok{*}\StringTok{ }\NormalTok{n}\OperatorTok{^}\NormalTok{(}\DecValTok{4}\OperatorTok{/}\DecValTok{45}\NormalTok{) }\CommentTok{# h*n^(-1/9)/n^(-1/5)}
\NormalTok{range.x <-}\StringTok{ }\KeywordTok{range}\NormalTok{(npden}\OperatorTok{$}\NormalTok{x) }\CommentTok{# Para fijar las posiciones de estimación}
\NormalTok{B <-}\StringTok{ }\DecValTok{1000}
\NormalTok{stat_den_boot <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\DataTypeTok{nrow =} \KeywordTok{length}\NormalTok{(npden}\OperatorTok{$}\NormalTok{x), }\DataTypeTok{ncol =}\NormalTok{ B)}
\ControlFlowTok{for}\NormalTok{ (k }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{B) \{}
    \CommentTok{# x_boot <- sample(x, n, replace = TRUE) + rnorm(n, 0, g)}
\NormalTok{    x_boot <-}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(n, }\KeywordTok{sample}\NormalTok{(x, n, }\DataTypeTok{replace =} \OtherTok{TRUE}\NormalTok{), g)}
\NormalTok{    den_boot <-}\StringTok{ }\KeywordTok{density}\NormalTok{(x_boot, }\DataTypeTok{bw =}\NormalTok{ h, }\DataTypeTok{from =}\NormalTok{ range.x[}\DecValTok{1}\NormalTok{], }\DataTypeTok{to =}\NormalTok{ range.x[}\DecValTok{2}\NormalTok{])}\OperatorTok{$}\NormalTok{y}
    \CommentTok{# Si se quiere tener en cuenta la variabilidad debida a la selección de}
    \CommentTok{# la ventana habría que emplear el mismo criterio en la función `density`.}
\NormalTok{    stat_den_boot[, k] <-}\StringTok{ }\NormalTok{den_boot }\OperatorTok{-}\StringTok{ }\NormalTok{npden}\OperatorTok{$}\NormalTok{y}
\NormalTok{\}}

\CommentTok{# Calculo del sesgo y error estándar }
\NormalTok{bias <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(stat_den_boot, }\DecValTok{1}\NormalTok{, mean)}
\NormalTok{std.err <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(stat_den_boot, }\DecValTok{1}\NormalTok{, sd)}

\CommentTok{# Representar estimación y corrección de sesgo bootstrap}
\KeywordTok{plot}\NormalTok{(npden, }\DataTypeTok{type=}\StringTok{"l"}\NormalTok{, }\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FloatTok{0.05}\NormalTok{), }\DataTypeTok{lwd =} \DecValTok{2}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(npden}\OperatorTok{$}\NormalTok{x, npden}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{bias)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{alfa <-}\StringTok{ }\FloatTok{0.05}
\NormalTok{pto_crit <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(stat_den_boot, }\DecValTok{1}\NormalTok{, quantile, }\DataTypeTok{probs =} \KeywordTok{c}\NormalTok{(alfa}\OperatorTok{/}\DecValTok{2}\NormalTok{, }\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{alfa}\OperatorTok{/}\DecValTok{2}\NormalTok{))}
\CommentTok{# ic_inf_boot <- npden$y - pto_crit[2, ]}
\NormalTok{ic_inf_boot <-}\StringTok{ }\KeywordTok{pmax}\NormalTok{(}\DecValTok{0}\NormalTok{, npden}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{pto_crit[}\DecValTok{2}\NormalTok{, ])}
\NormalTok{ic_sup_boot <-}\StringTok{ }\NormalTok{npden}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{pto_crit[}\DecValTok{1}\NormalTok{, ]}

\KeywordTok{plot}\NormalTok{(npden, }\DataTypeTok{type=}\StringTok{"l"}\NormalTok{, }\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FloatTok{0.05}\NormalTok{), }\DataTypeTok{lwd =} \DecValTok{2}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(npden}\OperatorTok{$}\NormalTok{x, }\KeywordTok{pmax}\NormalTok{(}\DecValTok{0}\NormalTok{, npden}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{bias))}
\KeywordTok{lines}\NormalTok{(npden}\OperatorTok{$}\NormalTok{x, ic_inf_boot, }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(npden}\OperatorTok{$}\NormalTok{x, ic_sup_boot, }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=0.7\linewidth]{06-npden_files/figure-latex/unnamed-chunk-4-1} \end{center}

\subsection{\texorpdfstring{Implementación con el paquete
\texttt{boot}}{Implementación con el paquete boot}}\label{implementaciuxf3n-con-el-paquete-boot}

Como también se comentó en la Sección \ref{modunif-boot-suav}, la
recomendación es implementar el bootstrap suavizado como un bootstrap
paramétrico:
...
\CommentTok{# hay que pasarlos a traves del argumento `data` de `boot`.}
\NormalTok{range.x <-}\StringTok{ }\KeywordTok{range}\NormalTok{(npden}\OperatorTok{$}\NormalTok{x)}
\NormalTok{data.precip <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ x, }\DataTypeTok{h =}\NormalTok{ h, }\DataTypeTok{range.x =}\NormalTok{ range.x)}

\NormalTok{ran.gen.smooth <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(data, mle) \{}
    \CommentTok{# Función para generar muestras aleatorias mediante}
    \CommentTok{# bootstrap suavizado con función núcleo gaussiana,}
    \CommentTok{# mle contendrá la ventana}
\NormalTok{    n <-}\StringTok{ }\KeywordTok{length}\NormalTok{(data}\OperatorTok{$}\NormalTok{x)}
\NormalTok{    g <-}\StringTok{ }\NormalTok{mle}
\NormalTok{    xboot <-}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(n, }\KeywordTok{sample}\NormalTok{(data}\OperatorTok{$}\NormalTok{x, n, }\DataTypeTok{replace =} \OtherTok{TRUE}\NormalTok{), g)}
\NormalTok{    out <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ xboot, }\DataTypeTok{h =}\NormalTok{ data}\OperatorTok{$}\NormalTok{h, }\DataTypeTok{range.x =}\NormalTok{ data}\OperatorTok{$}\NormalTok{range.x)}
\NormalTok{\}}

\NormalTok{statistic <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(data) }
                \KeywordTok{density}\NormalTok{(data}\OperatorTok{$}\NormalTok{x, }\DataTypeTok{bw =}\NormalTok{ data}\OperatorTok{$}\NormalTok{h, }\DataTypeTok{from =}\NormalTok{ range.x[}\DecValTok{1}\NormalTok{], }\DataTypeTok{to =}\NormalTok{ range.x[}\DecValTok{2}\NormalTok{])}\OperatorTok{$}\NormalTok{y}

\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{res.boot <-}\StringTok{ }\KeywordTok{boot}\NormalTok{(data.precip, statistic, }\DataTypeTok{R =}\NormalTok{ B, }\DataTypeTok{sim =} \StringTok{"parametric"}\NormalTok{,}
                 \DataTypeTok{ran.gen =}\NormalTok{ ran.gen.smooth, }\DataTypeTok{mle =}\NormalTok{ g)}

\CommentTok{# Calculo del sesgo y error estándar}
\NormalTok{bias <-}\StringTok{ }\KeywordTok{with}\NormalTok{(res.boot, }\KeywordTok{apply}\NormalTok{(t, }\DecValTok{2}\NormalTok{, mean, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{) }\OperatorTok{-}\StringTok{  }\NormalTok{t0)}
\NormalTok{std.err <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(res.boot}\OperatorTok{$}\NormalTok{t, }\DecValTok{2}\NormalTok{, sd, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{alfa <-}\StringTok{ }\FloatTok{0.05}
\NormalTok{pto_crit <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(res.boot}\OperatorTok{$}\NormalTok{t, }\DecValTok{2}\NormalTok{, quantile, }\DataTypeTok{probs =} \KeywordTok{c}\NormalTok{(alfa}\OperatorTok{/}\DecValTok{2}\NormalTok{, }\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{alfa}\OperatorTok{/}\DecValTok{2}\NormalTok{))}
\NormalTok{ic_inf_boot <-}\StringTok{ }\KeywordTok{pmax}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{2}\OperatorTok{*}\NormalTok{npden}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{pto_crit[}\DecValTok{2}\NormalTok{, ])}
\NormalTok{ic_sup_boot <-}\StringTok{ }\DecValTok{2}\OperatorTok{*}\NormalTok{npden}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{pto_crit[}\DecValTok{1}\NormalTok{, ]}

\KeywordTok{plot}\NormalTok{(npden, }\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FloatTok{0.05}\NormalTok{), }\DataTypeTok{lwd =} \DecValTok{2}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(npden}\OperatorTok{$}\NormalTok{x, }\KeywordTok{pmax}\NormalTok{(}\DecValTok{0}\NormalTok{, npden}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{bias))}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{matplot}\NormalTok{(npden}\OperatorTok{$}\NormalTok{x, }\KeywordTok{t}\NormalTok{(res.boot}\OperatorTok{$}\NormalTok{t), }\DataTypeTok{type =} \StringTok{"l"}\NormalTok{, }\DataTypeTok{col =} \StringTok{"darkgray"}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(npden, }\DataTypeTok{lwd =} \DecValTok{2}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(npden}\OperatorTok{$}\NormalTok{x, npden}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{bias)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{matplot}\NormalTok{(npden}\OperatorTok{$}\NormalTok{x, }\DecValTok{2}\OperatorTok{*}\NormalTok{npden}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\KeywordTok{t}\NormalTok{(res.boot}\OperatorTok{$}\NormalTok{t), }\DataTypeTok{type =} \StringTok{"l"}\NormalTok{, }\DataTypeTok{col =} \StringTok{"darkgray"}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(npden, }\DataTypeTok{lwd =} \DecValTok{2}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(npden}\OperatorTok{$}\NormalTok{x, npden}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{bias)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{est <-}\StringTok{ }\KeywordTok{approx}\NormalTok{(fit, }\DataTypeTok{xout =}\NormalTok{ x)}\OperatorTok{$}\NormalTok{y }\CommentTok{# est <- predict(fit)}
\NormalTok{resid <-}\StringTok{ }\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{est }\CommentTok{# resid <- residuals(fit)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{fit.sm}\OperatorTok{$}\NormalTok{sigma}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{n <-}\StringTok{ }\KeywordTok{length}\NormalTok{(x)}
\NormalTok{g <-}\StringTok{ }\NormalTok{h }\OperatorTok{*}\StringTok{ }\NormalTok{n}\OperatorTok{^}\NormalTok{(}\DecValTok{4}\OperatorTok{/}\DecValTok{45}\NormalTok{) }\CommentTok{# h*n^(-1/9)/n^(-1/5)}
\NormalTok{fit2 <-}\StringTok{ }\KeywordTok{locpoly}\NormalTok{(x, y, }\DataTypeTok{bandwidth =}\NormalTok{ g)}
\NormalTok{est2 <-}\StringTok{ }\KeywordTok{approx}\NormalTok{(fit2, }\DataTypeTok{xout =}\NormalTok{ x)}\OperatorTok{$}\NormalTok{y }\CommentTok{# est2 <- predict(fit2)}
\CommentTok{# resid2 <- y - est2 # resid2 <- residuals(fit2)}

\CommentTok{# Remuestreo}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{B <-}\StringTok{ }\DecValTok{1000}
\NormalTok{stat_fit_boot <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\DataTypeTok{nrow =} \KeywordTok{length}\NormalTok{(fit}\OperatorTok{$}\NormalTok{x), }\DataTypeTok{ncol =}\NormalTok{ B)}
\NormalTok{resid0 <-}\StringTok{ }\NormalTok{resid }\OperatorTok{-}\StringTok{ }\KeywordTok{mean}\NormalTok{(resid)}
\ControlFlowTok{for}\NormalTok{ (k }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{B) \{}
\NormalTok{    y_boot <-}\StringTok{ }\NormalTok{est2 }\OperatorTok{+}\StringTok{ }\KeywordTok{sample}\NormalTok{(resid0, }\DataTypeTok{replace =} \OtherTok{TRUE}\NormalTok{)}
\NormalTok{    fit_boot <-}\StringTok{ }\KeywordTok{locpoly}\NormalTok{(x, y_boot, }\DataTypeTok{bandwidth =}\NormalTok{ h)}\OperatorTok{$}\NormalTok{y}
\NormalTok{    stat_fit_boot[ , k] <-}\StringTok{ }\NormalTok{fit_boot }\OperatorTok{-}\StringTok{ }\NormalTok{fit}\OperatorTok{$}\NormalTok{y}
\NormalTok{\}}

\CommentTok{# Calculo del sesgo y error estándar }
\NormalTok{bias <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(stat_fit_boot, }\DecValTok{1}\NormalTok{, mean)}
\NormalTok{std.err <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(stat_fit_boot, }\DecValTok{1}\NormalTok{, sd)}

\CommentTok{# Representar estimación y corrección de sesgo bootstrap}
\KeywordTok{plot}\NormalTok{(x, y)}
\KeywordTok{lines}\NormalTok{(fit, }\DataTypeTok{lwd =} \DecValTok{2}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(fit}\OperatorTok{$}\NormalTok{x, fit}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{bias)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{alfa <-}\StringTok{ }\FloatTok{0.05}
\NormalTok{pto_crit <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(stat_fit_boot, }\DecValTok{1}\NormalTok{, quantile, }\DataTypeTok{probs =} \KeywordTok{c}\NormalTok{(alfa}\OperatorTok{/}\DecValTok{2}\NormalTok{, }\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{alfa}\OperatorTok{/}\DecValTok{2}\NormalTok{))}
\NormalTok{ic_inf_boot <-}\StringTok{ }\NormalTok{fit}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{pto_crit[}\DecValTok{2}\NormalTok{, ]}
\NormalTok{ic_sup_boot <-}\StringTok{ }\NormalTok{fit}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{pto_crit[}\DecValTok{1}\NormalTok{, ]}

\KeywordTok{plot}\NormalTok{(x, y)}
\KeywordTok{lines}\NormalTok{(fit, }\DataTypeTok{lwd =} \DecValTok{2}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(fit}\OperatorTok{$}\NormalTok{x, fit}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{bias)}
\KeywordTok{lines}\NormalTok{(fit}\OperatorTok{$}\NormalTok{x, ic_inf_boot, }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(fit}\OperatorTok{$}\NormalTok{x, ic_sup_boot, }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{# Remuestreo}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{n_pre <-}\StringTok{ }\KeywordTok{length}\NormalTok{(fit}\OperatorTok{$}\NormalTok{x)}
\NormalTok{stat_pred_boot <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\DataTypeTok{nrow =}\NormalTok{ n_pre, }\DataTypeTok{ncol =}\NormalTok{ B)}
\ControlFlowTok{for}\NormalTok{ (k }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{B) \{}
\NormalTok{    y_boot <-}\StringTok{ }\NormalTok{est2 }\OperatorTok{+}\StringTok{ }\KeywordTok{sample}\NormalTok{(resid0, }\DataTypeTok{replace =} \OtherTok{TRUE}\NormalTok{)}
\NormalTok{    fit_boot <-}\StringTok{ }\KeywordTok{locpoly}\NormalTok{(x, y_boot, }\DataTypeTok{bandwidth =}\NormalTok{ h)}\OperatorTok{$}\NormalTok{y}
\NormalTok{    pred_boot <-}\StringTok{ }\NormalTok{fit_boot }\OperatorTok{+}\StringTok{ }\KeywordTok{sample}\NormalTok{(resid0, n_pre, }\DataTypeTok{replace =} \OtherTok{TRUE}\NormalTok{)}
\NormalTok{    stat_pred_boot[ , k] <-}\StringTok{ }\NormalTok{pred_boot }\OperatorTok{-}\StringTok{ }\NormalTok{fit_boot}
\NormalTok{\}}

\CommentTok{# Cálculo de intervalos de predicción}
\CommentTok{# por el método percentil (básico)}
\NormalTok{alfa <-}\StringTok{ }\FloatTok{0.05}
\NormalTok{pto_crit_pred <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(stat_pred_boot, }\DecValTok{1}\NormalTok{, quantile, }\DataTypeTok{probs =} \KeywordTok{c}\NormalTok{(alfa}\OperatorTok{/}\DecValTok{2}\NormalTok{, }\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{alfa}\OperatorTok{/}\DecValTok{2}\NormalTok{))}
\NormalTok{ip_inf_boot <-}\StringTok{ }\NormalTok{fit}\OperatorTok{$}\NormalTok{y }\OperatorTok{+}\StringTok{ }\NormalTok{pto_crit_pred[}\DecValTok{1}\NormalTok{, ]}
\NormalTok{ip_sup_boot <-}\StringTok{ }\NormalTok{fit}\OperatorTok{$}\NormalTok{y }\OperatorTok{+}\StringTok{ }\NormalTok{pto_crit_pred[}\DecValTok{2}\NormalTok{, ]}

\KeywordTok{plot}\NormalTok{(x, y, }\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\OperatorTok{-}\DecValTok{150}\NormalTok{, }\DecValTok{75}\NormalTok{))}
\KeywordTok{lines}\NormalTok{(fit, }\DataTypeTok{lwd =} \DecValTok{2}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(fit}\OperatorTok{$}\NormalTok{x, fit}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{bias)}
\KeywordTok{lines}\NormalTok{(fit}\OperatorTok{$}\NormalTok{x, ic_inf_boot, }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(fit}\OperatorTok{$}\NormalTok{x, ic_sup_boot, }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(fit}\OperatorTok{$}\NormalTok{x, ip_inf_boot, }\DataTypeTok{lty =} \DecValTok{3}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(fit}\OperatorTok{$}\NormalTok{x, ip_sup_boot, }\DataTypeTok{lty =} \DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=0.7\linewidth]{07-npreg_files/figure-latex/unnamed-chunk-8-1} \end{center}

En este caso puede no ser recomendable considerar errores i.i.d., sería
de esperar heterocedásticidad (e incluso dependencia temporal). El
bootstrap residual se puede extender al caso heterocedástico y/o
dependencia (e.g.~Castillo-Páez \emph{et al.}, 2019, 2020).

\subsection{Wild bootstrap}\label{r-wild-bootstrap}

Como se describe en la Sección \ref{wild-bootstrap} en el caso
heterocedástico se puede emplear wild bootstrap.
...
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{B <-}\StringTok{ }\DecValTok{1000}
\NormalTok{fit_boot <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\DataTypeTok{nrow =}\NormalTok{ n_pre, }\DataTypeTok{ncol =}\NormalTok{ B)}
\ControlFlowTok{for}\NormalTok{ (k }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{B) \{}
\NormalTok{        rwild <-}\StringTok{ }\KeywordTok{sample}\NormalTok{(}\KeywordTok{c}\NormalTok{((}\DecValTok{1} \OperatorTok{-}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(}\DecValTok{5}\NormalTok{))}\OperatorTok{/}\DecValTok{2}\NormalTok{, (}\DecValTok{1} \OperatorTok{+}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(}\DecValTok{5}\NormalTok{))}\OperatorTok{/}\DecValTok{2}\NormalTok{), n, }\DataTypeTok{replace =} \OtherTok{TRUE}\NormalTok{, }
                        \DataTypeTok{prob =} \KeywordTok{c}\NormalTok{((}\DecValTok{5} \OperatorTok{+}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(}\DecValTok{5}\NormalTok{))}\OperatorTok{/}\DecValTok{10}\NormalTok{, }\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{(}\DecValTok{5} \OperatorTok{+}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(}\DecValTok{5}\NormalTok{))}\OperatorTok{/}\DecValTok{10}\NormalTok{))}
\NormalTok{    y_boot <-}\StringTok{ }\NormalTok{est2 }\OperatorTok{+}\StringTok{ }\NormalTok{resid}\OperatorTok{*}\NormalTok{rwild}
\NormalTok{    fit_boot[ , k] <-}\StringTok{ }\KeywordTok{locpoly}\NormalTok{(x, y_boot, }\DataTypeTok{bandwidth =}\NormalTok{ h)}\OperatorTok{$}\NormalTok{y}
    \CommentTok{# OJO: bootstrap percetil directo}
\NormalTok{\}}
        

\CommentTok{# Calculo del sesgo y error estándar}
\NormalTok{bias <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(fit_boot, }\DecValTok{1}\NormalTok{, mean, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{) }\OperatorTok{-}\StringTok{  }\NormalTok{fit}\OperatorTok{$}\NormalTok{y}
\NormalTok{std.err <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(fit_boot, }\DecValTok{1}\NormalTok{, sd, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{)}
...
\subsection{Distribución asintótica del estimador de
Kaplan-Meier}\label{distribuciuxf3n-asintuxf3tica-del-estimador-de-kaplan-meier}
...
habituales de datos
dependientes}\label{introducciuxf3n-a-las-condiciones-de-dependencia-y-modelos-habituales-de-datos-dependientes}
...
dependencia}\label{modelos-paramuxe9tricos-de-dependencia}
...
dependientes}\label{el-bootstrap-en-la-estimaciuxf3n-con-datos-dependientes}
...
\subsection{Modelos paramétricos de
dependencia}\label{modelos-paramuxe9tricos-de-dependencia-1}
...
\(b\)}{Elección de b}}\label{elecciuxf3n-de-b}
...
\subsection{El método del
submuestreo}\label{el-muxe9todo-del-submuestreo}
...
\section{\texorpdfstring{Implementación en \texttt{R} con el paquete
\texttt{forecast}}{Implementación en R con el paquete forecast}}\label{implementaciuxf3n-en-r-con-el-paquete-forecast}
...
error variance in bivariate nonparametric regression. \emph{Statistics
and Computing} \textbf{17}, 39-47.

Bowman A.W. and Azzalini A. (2019). \emph{R package `sm': nonparametric
smoothing methods} (version 2.2).
...
\chapter{Introducción al procesamiento en paralelo en
R}\label{intro-hpc}

En este apéndice se pretenden mostrar las principales herramientas para
el procesamiento en paralelo disponibles en R y dar una idea de su
funcionamiento. Para más detalles se recomienda ver
\href{https://cran.r-project.org/view=HighPerformanceComputing}{CRAN
Task View: High-Performance and Parallel Computing with R} (además de
HPC, High Performance Computing, también incluye herramientas para
computación distribuida)\footnote{También puede ser de interés la
  presentación
  \href{https://www.r-users.gal/sites/default/files/10_aurelio_rodriguez.pdf}{R
  y HPC (uso de R en el CESGA)} de Aurelio Rodríguez en las
  \href{https://www.r-users.gal/pagina/programa-2018}{VI Xornada de
  Usuarios de R en Galicia}}.
...
\begin{itemize}
\item
  \textbf{Núcleo}: término empleado para referirse a un procesador
  lógico de un equipo (un equipo puede tener un único procesador con
  múltiples núcleos que pueden realizar operaciones en paralelo).
  También podría referirse aquí a un equipo (nodo) de una red (clúster
  de equipos; en la práctica cada uno puede tener múltiples núcleos).
  \emph{Un núcleo puede ejecutar procesos en serie}.
\item
...
Por defecto la versión oficial de R emplea un único núcleo, aunque se
puede compilar de forma que realice cálculos en paralelo (e.g.~librería
LAPACK). También están disponibles otras versiones de R que ya
implementan por defecto procesamiento en paralelo
(\href{https://mran.revolutionanalytics.com/documents/rro/multithread}{multithread}):

\begin{itemize}
\tightlist
\item
  \href{https://mran.revolutionanalytics.com}{Microsoft R Open}: versión
  de R con rendimiento mejorado.
\end{itemize}

Métodos simples de paralelización\footnote{Realmente las herramientas
  estándar son \emph{OpenMP} para el procesamiento en paralelo con
  memoria compartida en un único equipo y \emph{MPI} para la computación
  distribuida en múltiples nodos.}:

\begin{itemize}
\item
  \emph{Forking}: Copia el proceso de R a un nuevo núcleo (se comparte
  el entorno de trabajo). Es el más simple y eficiente pero \textbf{no
  está disponible en Windows}.
\item
  \emph{Socket}: Lanza una nueva versión de R en cada núcleo, como si se
  tratase de un cluster de equipos comunicados a traves de red (hay que
  crear un entorno de trabajo en cada núcleo). Disponible en todos los
  sistemas operativos.
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{func <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(k) \{}
\NormalTok{  i_boot <-}\StringTok{ }\KeywordTok{sample}\NormalTok{(}\KeywordTok{nrow}\NormalTok{(iris), }\DataTypeTok{replace =} \OtherTok{TRUE}\NormalTok{)}
  \KeywordTok{lm}\NormalTok{(Petal.Width }\OperatorTok{~}\StringTok{ }\NormalTok{Petal.Length, }\DataTypeTok{data =}\NormalTok{ iris[i_boot, ])}\OperatorTok{$}\NormalTok{coefficients}
\NormalTok{\}}

\KeywordTok{RNGkind}\NormalTok{(}\StringTok{"L'Ecuyer-CMRG"}\NormalTok{) }\CommentTok{# Establecemos Pierre L'Ecuyer's RngStreams...}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}

\KeywordTok{system.time}\NormalTok{(res.boot <-}\StringTok{ }\KeywordTok{mclapply}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{100}\NormalTok{, func)) }\CommentTok{# En Windows llama a lapply() (mc.cores = 1)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{getSimulation <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(isim, }\DataTypeTok{B =} \DecValTok{2000}\NormalTok{, }\DataTypeTok{n =} \DecValTok{30}\NormalTok{, }\DataTypeTok{alfa =} \FloatTok{0.1}\NormalTok{, }\DataTypeTok{mu =} \DecValTok{100}\NormalTok{) \{}
\NormalTok{    rate <-}\StringTok{ }\DecValTok{1}\OperatorTok{/}\NormalTok{mu }\CommentTok{# 0.01}
\NormalTok{    resnames <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Cobertura"}\NormalTok{, }\StringTok{"Longitud"}\NormalTok{)}
    \CommentTok{# intervals <- c("Normal", "Percentil", "Percentil-t", "Percentil-t simetrizado")}
...
\NormalTok{    resultados <-}\StringTok{ }\KeywordTok{array}\NormalTok{(}\DataTypeTok{dim =} \KeywordTok{c}\NormalTok{(}\KeywordTok{length}\NormalTok{(resnames), }\KeywordTok{length}\NormalTok{(intervals)))}
    \KeywordTok{dimnames}\NormalTok{(resultados) <-}\StringTok{ }\KeywordTok{list}\NormalTok{(resnames, intervals)}
    \CommentTok{# for (isim in 1:nsim) \{ # isim <- 1}
\NormalTok{    muestra <-}\StringTok{ }\KeywordTok{rexp}\NormalTok{(n, }\DataTypeTok{rate =} \FloatTok{0.01}\NormalTok{)}
\NormalTok{    media <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(muestra)}
\NormalTok{    desv <-}\StringTok{ }\KeywordTok{sd}\NormalTok{(muestra)}
    \CommentTok{# boot()}
