...
Como ejemplo ilustrativo consideramos los datos simulados `r latexfig("muestra-sim")`:
...
Alternativamente podríamos asumir un modelo paramétrico y estimar los parámetros a partir de la muestra `r latexfig("muestra-sim-aprox")`.
...
```{example, label="media-dt-conocida", name="Inferencia sobre la media con varianza conocida"}
...
```
`r latexfig("microorganismos", output = is_latex())`
```{r microorganismos, fig.cap="Distribución del tiempo de vida de microorganismos."}
...
`r citefig("estad-boot")`.

```{r estad-boot, fig.cap='Distribución del estadístico boostrap y aproximaciones de los cuantiles. Con línea discontinua se muestra la distribución normal asintótica.'}
...
```{example, label="media-dt-conocida-perturbando", name="Inferencia sobre la media con varianza conocida, continuación"}
```
...
```{example, label="mediana", name="Inferencia sobre la mediana"}
<br> \vspace{0.5cm}
...
```{example, label="media3", name="Media muestral para una muestra de tamaño 3"}
<br> \vspace{0.5cm}
...
```{example, label="media-dt-desconocida", name="Inferencia sobre la media con varianza desconocida"}
<br> \vspace{0.5cm}
...
```{example, label="media-dt-desconocida-boot", name="Inferencia sobre la media con varianza desconocida, continuación"}
<br> \vspace{0.5cm}
